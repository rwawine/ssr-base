...
export const optimizeImageUrl = (url: string, options: ImageOptimizationOptions = {}) => {
    ...
    if (url.includes('res.cloudinary.com')) {
        const baseUrl = url.replace('/upload/', `/upload/w_${width},c_scale,f_${format},q_${quality},dpr_auto,fl_progressive/`);
        return baseUrl;
    }
...
export const createSrcSet = (imageData: any, sizes: number[] = [300, 600, 900, 1200]) => {
    if (!imageData?.url) return '';

    return sizes
        .map(size => {
            const optimizedUrl = optimizeImageUrl(imageData.url, { width: size, quality: 80 });
...
export const shouldPrioritizeImage = (index: number, totalImages: number, isAboveFold: boolean = false) => {
...
export const createSizesAttribute = (breakpoints: { [key: string]: string } = {}) => {
...
export const optimizeForLCP = (imageData: any, options: {
    isLCP?: boolean;
