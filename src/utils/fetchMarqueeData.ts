...
const MARQUEE_API_URL =
  "https://admin.dilavia.by/api/karusel-s-preimushhestvami?populate[0]=item&populate[1]=item.imageUrl";
...
export function extractImagesFromMarqueeData(
  data: MarqueeApiResponse,
): string[] {
  return data.data.item
    .filter(
      (item): item is import("@/types/marquee").MarqueeSlideItem =>
...
export function extractTitlesFromMarqueeData(
  data: MarqueeApiResponse,
): string[] {
  return data.data.item
    .filter(
      (item): item is import("@/types/marquee").MarqueeTitleItem =>
