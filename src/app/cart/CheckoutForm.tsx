...
const CheckoutForm = forwardRef<HTMLFormElement, any>((props, ref) => {
  ...
  useImperativeHandle(ref, () => {
    const form = formElement.current as HTMLFormElement & { submitForm?: () => void };
    if (form) {
      form.submitForm = () => {
  ...
  const validate = () => {
    const errs: any = {};
    if (!/^375\d{9}$/.test(form.phone.replace(/\D/g, ''))) errs.phone = 'Введите корректный телефон';
    if (!form.name.trim()) errs.name = 'Введите имя';
    if (!/^\S+@\S+\.\S+$/.test(form.email)) errs.email = 'Введите корректный email';
    if (form.delivery === 'courier' && !form.address.trim()) errs.address = 'Введите адрес';
    return errs;
  };
    ...
    <form ref={formElement} className={styles.checkoutSection} onSubmit={handleSubmit} autoComplete="off">
      ...
      <div className={styles.field}>
        ...
        <input
          ...
          onChange={e => {
            ...
            value = '+375' + value.replace(/\D/g, '').replace(/^375/, '').slice(0, 9);
      ...
      <div className={styles.field}>
        ...
        <div className={styles.deliveryTabs}>
          <button
            type="button"
            className={form.delivery === 'courier' ? styles.buttonActive : styles.buttonTab}
            onClick={() => setForm(f => ({ ...f, delivery: 'courier' }))}
          >Курьер</button>
          <button
            type="button"
            className={form.delivery === 'pickup' ? styles.buttonActive : styles.buttonTab}
