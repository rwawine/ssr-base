...
import PopularProductSliderClient from './PopularProductSliderClient';

export interface PopularProductSliderProps {
  products: Product[];
  title?: string;
  description?: string;
}
...
export default function PopularProductSlider({ products, title, description }: PopularProductSliderProps) {
  // Показываем первые 4 товара для SSR
  ...
  return (
    <>
      <div className={styles.titleRow}>
        <div>
          <h2 className={styles.title}>{title || 'Диваны в Минске'}</h2>
      ...
      <PopularProductSliderClient products={products} />
    </>
  );
} 