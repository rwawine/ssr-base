...
interface PopularProductSliderSSRProps {
  products: Product[];
  title?: string;
  description?: string;
}

export default function PopularProductSliderSSR({ products, title, description }: PopularProductSliderSSRProps) {
  ...
  // Показываем fallback до инициализации на клиенте
  if (!isClient) {
    return (
      <>
        <div className={styles.titleRow}>
          <div>
            ...
            <div className={styles.description}>
              {description || 'В интернет-магазине Dilavia.by представлены только тщательно отобранные модели диванов, мягкой и корпусной мебели. Каждая модель из нашего интернет каталога – это воплощение отличного дизайна, легенда мебельной отрасли.'}
            </div>
          </div>
          <div className={styles.arrowGroup}>
            <button
              className={`${styles.arrowBtn} ${styles.arrowBtnInline} ${styles.left}`}
              aria-label="Назад"
              disabled
              aria-disabled="true"
            >
            ...
            <button
              className={`${styles.arrowBtn} ${styles.arrowBtnInline} ${styles.right}`}
              aria-label="Вперёд"
              disabled
              aria-disabled="true"
            >
