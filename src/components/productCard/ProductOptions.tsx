...
export default function ProductOptions({
  ...
  const isAdditionalOptionSelected = (option: AdditionalOption) => {
    return selectedAdditionalOptions.some(
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        dimDropdownRef.current &&
        !dimDropdownRef.current.contains(event.target as Node)
      ) {
        setIsDimOpen(false);
      }
      if (
        optDropdownRef.current &&
        !optDropdownRef.current.contains(event.target as Node)
      ) {
        setIsOptOpen(false);
      }
    };
            ...
            {isOptOpen && (
              <ul className={styles.dropdownList}>
                {availableAdditionalOptions.map((option, index) => (
                  <li
                    key={
                      option.name ? `opt-${option.name}` : `opt-idx-${index}`
                    }
                    className={styles.dropdownItemCheckbox}
                    onClick={() => onAdditionalOptionToggle(option)}
                  >
                    <div
                      className={`${styles.checkbox} ${isAdditionalOptionSelected(option) ? styles.checkboxSelected : ""}`}
                    >
                      {isAdditionalOptionSelected(option) && (
                        <svg viewBox="0 0 16 16" fill="white">
                          <path d="M4 8.2l2.4 2.4L12 5" />
                        </svg>
                      )}
                    </div>
